...
bool intersect(const Fobb& box, const Fvector& origin, const Fvector& direction, float& dist, Fvector& norm)
{
    ...
    if (intersect(kOrigin, kDirection, box.m_halfsize, fT0, fT1, ax0, ax1))
    {
        ...
        if (bPick)
        {
            ...
            case a_py: norm.set(box.m_rotate.j); break;
            case a_ny: norm.invert(box.m_rotate.j); break;
            case a_pz: norm.set(box.m_rotate.k); break;
            case a_nz: norm.invert(box.m_rotate.k); break;
...
bool bone_intersect(
    u16 bone, const IKinematics& K, const Fvector& origin, const Fvector& direction, float& dist, Fvector& norm)
{
...
bool __stdcall intersect(
    const IKinematics& K, const Fvector& origin, const Fvector& direction, u16& bone_id, float& dist, Fvector& norm)
{
    bool b_res = false;
...
bool __stdcall intersect(const Fmatrix& object_transform, const IKinematics& K, const Fvector& origin,
    const Fvector& direction, u16& bone_id, float& dist, Fvector& norm)
{
