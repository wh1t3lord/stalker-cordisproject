...
void SDK_Cache::MakeCache(void) { this->MakeStaticGeometryCache(); }

void SDK_Cache::UpdateCache(void) { this->UpdateStaticGeometryCache(); }

void SDK_Cache::MakeStaticGeometryCache(void)
{
    for (std::filesystem::directory_entry object : std::filesystem::directory_iterator(
             "rawdata\\Static Geometry")) // Подумать над тем, когда мы будем вызывать из под папки
    {
...
bool SDK_Cache::IsSameObjectContains(void)
{
    return false;
}
