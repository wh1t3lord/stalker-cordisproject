...
public:
    PropItem(EPropType _type)
        : type(_type), prop_color(0), val_color(0), item(nullptr), key(nullptr), OnClickEvent(nullptr),
          OnDrawTextEvent(nullptr), OnItemFocused(nullptr)
    {
        m_Flags.zero();
    }
    virtual ~PropItem()
...
public:
    ...
    ChooseValue(shared_str* val, u32 cid, LPCSTR path, void* param, u32 sub_item_count, u32 choose_flags)
        : RTextValue(val), m_ChooseID(cid), m_StartPath(path), subitem(sub_item_count), m_Items(nullptr),
          m_FillParam(param), OnChooseFillEvent(nullptr), OnDrawThumbnailEvent(nullptr), m_ChooseFlags(choose_flags)
    {
    }
};
