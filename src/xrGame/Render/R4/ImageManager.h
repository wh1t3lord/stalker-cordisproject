...
inline bool operator==(const SSimpleImage& a, shared_str nm) { return a.name == nm; }

inline bool operator<(const SSimpleImage& a, const SSimpleImage& b) { return a.name < b.name; }
...
public:
    ...
    void SynchronizeTextures(bool sync_thm, bool sync_game, bool bForceGame, FS_FileSet* source_map,
        xr_vector<xr_string>* sync_list_without_extention, FS_FileSet* modif_map = 0, bool bForceBaseAge = false);
    ...
    void CreateTextureThumbnail(
        ETextureThumbnail* THM, const xr_string& src_name, LPCSTR path = 0, bool bSetDefParam = true);
    BOOL CreateOBJThumbnail(LPCSTR tex_name, CEditableObject* obj, int age);
    void CreateLODTexture(
        CEditableObject* object, U32Vec& lod_pixels, U32Vec& nm_pixels, u32 tgt_w, u32 tgt_h, int samples, int quality);
    void CreateLODTexture(
        CEditableObject* object, LPCSTR tex_name, u32 tgt_w, u32 tgt_h, int samples, int age, int quality);
    ...
    void MergedTextureRemapUV(float& dest_u, float& dest_v, float src_u, float src_v, const Fvector2& offs,
        const Fvector2& scale, bool bRotate);
    int CreateMergedTexture(const RStringVec& src_names, LPCSTR dest_name, STextureParams::ETFormat fmt, int dest_width,
        int dest_height, Fvector2Vec& dest_offset, Fvector2Vec& dest_scale, boolVec& dest_rotate, U32Vec& remap);
    int CreateMergedTexture(const RStringVec& src_names, LPCSTR dest_name, STextureParams::ETFormat fmt,
        int dest_width_min, int dest_width_max, int dest_height_min, int dest_height_max, Fvector2Vec& dest_offset,
        Fvector2Vec& dest_scale, boolVec& dest_rotate, U32Vec& remap);
    int CreateMergedTexture(u32 layer_cnt, SSimpleImageVec& src_images, SSimpleImage& dst_image, int dest_width,
        int dest_height, Fvector2Vec& dest_offset, Fvector2Vec& dest_scale, boolVec& dest_rotate, U32Vec& remap);
    int CreateMergedTexture(u32 layer_cnt, SSimpleImageVec& src_images, SSimpleImage& dst_image, int dest_width_min,
        int dest_width_max, int dest_height_min, int dest_height_max, Fvector2Vec& dest_offset, Fvector2Vec& dest_scale,
    ...
    void RefreshTextures(xr_vector<xr_string>* modif);
...
extern __declspec(dllexport) CImageManager ImageLib;
