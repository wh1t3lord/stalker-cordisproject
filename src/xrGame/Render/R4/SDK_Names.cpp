	...
	namespace SDK
	{
		...
		void SDK_Names::Initialize(void)
		{
			if (SDK_FileSystemState::m_is_translationempty)
			{
				...
				Cordis::XML::CordisXmlDocument document;
				...
				Cordis::XML::CordisXmlElement* element_root = document.RootElement();
				if (element_root)
				{
					Cordis::XML::CordisXmlElement* element_string = element_root->FirstChildElement();
					if (element_string)
					{
		...
		bool SDK_Names::LoadXml(const xr_string& filepath)
		{
			...
			Cordis::XML::CordisXmlDocument document(filepath.c_str());

			std::pair<xr_string, xr_map<xr_string, xr_string>> data;
			std::pair<xr_string, xr_string> data_map;

			if (document.LoadFile(Cordis::XML::CordisXml_ENCODING_UTF8))
			{
				Cordis::XML::CordisXmlElement* element_root = document.RootElement();
