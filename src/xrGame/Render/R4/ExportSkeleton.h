...
class CSurface;
class CInifile;
__declspec(dllexport) extern BOOL g_force16BitTransformQuant;
__declspec(dllexport) extern float g_EpsSkelPositionDelta;
 

 

struct SSkelVert : public st_SVert
{
    Fvector tang;
    Fvector binorm;
    SSkelVert()
...
struct __declspec(dllexport) SSkelFace
{
    WORD v[3];
};
...
class __declspec(dllexport) CSkeletonCollectorPacked
{
protected:
    SkelVertVec m_Verts;
    SkelFaceVec m_Faces;
...
public:
    ...
    bool add_face(SSkelVert& v0, SSkelVert& v1, SSkelVert& v2)
    {
        ...
        {
            //   ELog.Msg(mtError, "Degenerate face found. Removed."); Lord: Interpret
            invalid_faces++;
            return false;
        }
        SSkelFace F;
        ...
        {
            //  ELog.Msg(mtError, "Duplicate face found. Removed."); Lord: Interpret
            invalid_faces++;
            return false;
        }
    }
...
class __declspec(dllexport) CExportSkeleton :
...
void __declspec(dllexport) ComputeOBB_RAPID(Fobb& B, FvectorVec& V, u32 t_cnt);
void __declspec(dllexport) ComputeOBB_WML(Fobb& B, FvectorVec& V);
