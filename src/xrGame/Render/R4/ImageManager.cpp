...
//extern FIBITMAP* Surface_Load(char* full_name); Lord: подумать над реализацией данного метода, можно ли только ограничиться else конструкцией

extern "C" __declspec(dllimport) int DXTCompress(
    LPCSTR out_name, u8* raw_data, u8* ext_data, u32 w, u32 h, u32 pitch, STextureParams* options, u32 depth);
...
bool Surface_Load(LPCSTR full_name, U32Vec& data, u32& w, u32& h,
    u32& a) // Lord: подумать здесь, вроде бы как CImage не реализован у ПЫС
{
    ...
    xr_string ext = EFS.ExtractFileExt(full_name); // Lord: здесь должен был быть LowerCase подумать
    if (ext == ".tga")
    {
        CImage img;
...
void CImageManager::CreateTextureThumbnail(
    ETextureThumbnail* THM, const xr_string& src_name, LPCSTR initial, bool bSetDefParam)
{
...
bool CImageManager::MakeGameTexture(ETextureThumbnail* THM, LPCSTR game_name, u32* load_data)
{
    ...
    // remove old
    FS.file_delete(game_name);
    xr_string game_name2 = EFS.ChangeFileExt(game_name, "#.dds");
    ...
    if ((THM->m_TexParams.type == STextureParams::ttBumpMap) && (THM->m_TexParams.ext_normal_map_name.size()))
    {
        ...
        if (NM_THM->_Format().type == STextureParams::ttNormalMap)
        {
            ...
            {
                //   ELog.DlgMsg(mtError, "Invalid special normal map format '%s'. It should be '32 bit (8:8:8:8)'",
                //   e_name);
                e_res = false;
            }
        }
    ...
    if (1 != res)
    {
        ...
        switch (res)
        {
        case 0: break; // ELog.DlgMsg(mtError, "Can't make game texture '%s'.", THM->m_SrcName.c_str()); break;
        case -1000: break; // ELog.Msg(mtError, "Invalid gloss mask '%s'.", THM->m_SrcName.c_str()); return true;
        }
        return false;
    }
    R_ASSERT((res == 1) && FS.file_length(game_name));
...
bool CImageManager::LoadTextureData(LPCSTR src_name, U32Vec& data, u32& w, u32& h, int* age)
{
    ...
    FS.update_path(fn, _game_textures_, EFS.ChangeFileExt(src_name, ".dds").c_str());
    u32 a;
 ...
 //   SPBItem* pb = 0;
  ...
  //  if (bProgress)
   //     pb = UI->ProgressStart(M_BASE.size(), "Synchronize textures...");
    ...
    //    if (UI->NeedAbort()) Lord: посмотреть что за метод
      //      break;

//        if (bProgress) Lord: Interpret
    //        pb->Inc(
...
BOOL CImageManager::CreateOBJThumbnail(LPCSTR tex_name, CEditableObject* obj, int age) // Lord: подумать над реализацией
{
    BOOL bResult = TRUE;
    // save render params
...
//    if (Device.MakeScreenshot(pixels, w, h)) // Lord: добавить подобную реализацию только просто через SDKUI::MakeScreenShot 
    {
        EObjectThumbnail tex(tex_name, false);
    //    tex.CreateFromData(&*pixels.begin(), w, h, obj->GetFaceCount(), obj->GetVertexCount()); Lord: добавить CEditableObject
        tex.Save(age);
    }
 //   else
    {
...
BOOL CImageManager::CreateSmallerCubeMap(LPCSTR src_name, LPCSTR dst_name)
{
    ...
    /* Lord: подумать над методом Surface_Load
