...
public:
    ...
    Script_SchemeXRHit(CScriptGameObject* const p_client_object, DataBase::Script_ComponentScheme_XRHit* storage);
        ...
        const xr_string& scheme_name, const xr_string& section_name, DataBase::Script_IComponentScheme* storage)
    {
        ...
        Script_ISchemeEntity* p_scheme = new Script_SchemeXRHit(p_client_object, static_cast<DataBase::Script_ComponentScheme_XRHit*>(storage));
...
private:
    DataBase::Script_ComponentScheme_XRHit* m_p_storage;
};
} // namespace Scripts
} // namespace Cordis
