...
void CPurchaseList::process(CInifile& ini_file, LPCSTR section, CInventoryOwner& owner)
{
    ...
    for (; I != E; ++I)
    {
        VERIFY3(((*I).second.empty() == false), "PurchaseList : cannot handle lines in section without values", section);

        string256 temp0, temp1;
        THROW3(_GetItemCount((*I).second.c_str()) == 2, "Invalid parameters in section", section);
        process(game_object, (*I).first.c_str(), atoi(_GetItem((*I).second.c_str(), 0, temp0)),
