...
namespace Scripts
{
Script_SE_Physic::Script_SE_Physic(LPCSTR section_name) : inherited(section_name), m_is_secret_item(false) 
{ Msg("[Scripts/Script_SE_Physic/ctor(section_name)] %s", section_name); }

Script_SE_Physic::~Script_SE_Physic(void) {}

void Script_SE_Physic::on_register(void) 
{
...
void Script_SE_Physic::on_unregister(void) 
{
    Script_StoryObject::getInstance().unregistrate_by_id(this->ID);
    inherited::on_unregister();
}

bool Script_SE_Physic::can_switch_online(void) const
{ 
