...
void Script_SchemeSRPsyAntenna::update(const float delta)
{
    ...
    if (XR_LOGIC::try_switch_to_another_section(this->m_npc, *this->m_storage, p_client_object))
    {
        return;
    }
...
void Script_SchemeSRPsyAntenna::zone_enter(void)
{
    ...
    float sound_intensity_base =
        this->m_manager_psy_antenna->getSoundIntensityBase() + this->m_storage->getSRPsyAntennaIntensity();
    this->m_manager_psy_antenna->setSoundIntensityBase(sound_intensity_base);

    float mute_sound_threshold =
        this->m_manager_psy_antenna->getMuteSoundThreshold() + this->m_storage->getSRPsyAntennaMuteSoundThreshold();
    this->m_manager_psy_antenna->setMuteSoundThreshold(mute_sound_threshold);

    float hit_intensity = this->m_manager_psy_antenna->getHitIntensity() + this->m_storage->getSRPsyAntennaIntensity();
    ...
    float phantom_spawn_probability = this->m_manager_psy_antenna->getPhantomSpawnProbability() +
        this->m_storage->getSRPsyAntennaPhantomProbability();
    this->m_manager_psy_antenna->setPhantomSpawnProbability(phantom_spawn_probability);

    this->m_manager_psy_antenna->setNoStatic(this->m_storage->IsSRPsyAntennaNoStatic());
    this->m_manager_psy_antenna->setNoMumble(this->m_storage->IsSRPsyAntennaNoMumble());
    this->m_manager_psy_antenna->setHitTypeName(this->m_storage->getSRPsyAntennaHitTypeName());
    this->m_manager_psy_antenna->setHitFrequency(this->m_storage->getSRPsyAntennaHitFrequency());

    if (this->m_storage->getSRPsyAntennaPostProcessName().empty() ||
        this->m_storage->getSRPsyAntennaPostProcessName() == "nil")
    {
        Msg("[Scripts/Script_SchemeSRPsyAntenna/update(delta)] WARNING: postprocess is 'nil' return!");
        return;
    }

    if (this->m_manager_psy_antenna->getPostProcess().at(this->m_storage->getSRPsyAntennaPostProcessName()) ==
    ...
    float intensity_base = std::get<_POSTPROCESS_GET_INTENSITY_BASE>(this->m_manager_psy_antenna->getPostProcess().at(
                               this->m_storage->getSRPsyAntennaPostProcessName())) +
        this->m_storage->getSRPsyAntennaIntensity();

    this->m_manager_psy_antenna->setPostProcessIntensityBase(
        this->m_storage->getSRPsyAntennaPostProcessName(), intensity_base);
}
...
void Script_SchemeSRPsyAntenna::zone_leave(void)
{
    ...
    float hit_intensity =
        this->m_manager_psy_antenna->getHitIntensity() - this->m_storage->getSRPsyAntennaHitIntensity();
    this->m_manager_psy_antenna->setHitIntensity(hit_intensity);
    ...
    if (this->m_storage->getSRPsyAntennaPostProcessName() == "nil" ||
        this->m_storage->getSRPsyAntennaPostProcessName().empty())
    {
        ...
        std::tuple<float, float, std::uint32_t>(0.0f, 0.0f, 0))
    {
        float intensity_base =
            std::get<_POSTPROCESS_GET_INTENSITY_BASE>(
                this->m_manager_psy_antenna->getPostProcess().at(this->m_storage->getSRPsyAntennaPostProcessName())) -
            this->m_storage->getSRPsyAntennaIntensity();
        this->m_manager_psy_antenna->setPostProcessIntensityBase(
            this->m_storage->getSRPsyAntennaPostProcessName(), intensity_base);
    }
