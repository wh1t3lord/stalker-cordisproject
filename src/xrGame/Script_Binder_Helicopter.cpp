...
namespace Cordis
{
	namespace Scripts
	{
		...
		Script_Binder_Helicopter::Script_Binder_Helicopter(CScriptGameObject* const p_client_object) : CScriptBinderObject(p_client_object), m_is_loaded(false), m_is_initialized(false), m_p_helicopter(Script_HelicopterFireStorage::getInstance().AllocateFirer(p_client_object)), m_p_ini(p_client_object->spawn_ini())
		{
		}
		...
		bool Script_Binder_Helicopter::net_Spawn(SpawnType DC)
		{
			return false;
		}
		...
		bool Script_Binder_Helicopter::net_SaveRelevant()
		{
			return true;
		}
	}
