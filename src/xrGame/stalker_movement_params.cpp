...
struct stalker_movement_loophole_id_predicate
{
    shared_str m_id;

    stalker_movement_loophole_id_predicate(shared_str const& id) : m_id(id) {}
    bool operator()(smart_cover::loophole* loophole) const { return (loophole->id()._get() == m_id._get()); }
...
void stalker_movement_params::cover_loophole_id(shared_str const& loophole_id)
{
    ...
    Loopholes::const_iterator i =
        std::find_if(loopholes.begin(), loopholes.end(), stalker_movement_loophole_id_predicate(loophole_id));
