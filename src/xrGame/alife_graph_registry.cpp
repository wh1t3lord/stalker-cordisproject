...
{
#ifdef DEBUG
        MESSAGE("[LSS] Attaching item [%s][%d] to [%s][%d]", item->base()->name_replace(), item->base()->ID,
...
{
#ifdef DEBUG
        MESSAGE("[LSS] Detaching item [%s][%d] from [%s][%d]", item->base()->name_replace(), item->base()->ID,
            object.name_replace(), object.ID);
#endif
    if (alife_query)
...
{
#ifdef DEBUG
        MESSAGE("[LSS] adding object [%s][%d] to graph point %d", object->name_replace(), object->ID, game_vertex_id);
#endif
...
void CALifeGraphRegistry::remove(CSE_ALifeDynamicObject* object, GameGraph::_GRAPH_ID game_vertex_id, bool update)
{
    ...
    {
#ifdef DEBUG
            MESSAGE("[LSS] removing object [%s][%d] from graph point %d", object->name_replace(), object->ID,
                game_vertex_id);
#endif
